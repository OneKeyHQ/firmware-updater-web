...
function matchesPackageFile(file: File, fileNamePrefix: string) {
  ...
  return (
    fileName === `${prefix}.okpkg` || fileName.startsWith(`${prefix}-resource-`)
  );
}
...
export function matchPro2ResourcePackageDirectory(
  ...
  const packageFiles = selectedFiles.filter((file) =>
    file.name.toLowerCase().endsWith('.okpkg')
  );
  const matchedFiles: Record<string, File> = {};

  for (const slot of slots) {
    const matches = packageFiles.filter((file) =>
      matchesPackageFile(file, slot.fileNamePrefix)
    );
    if (matches.length === 0) {
